"""Shared base classes for the CUT family of models.

The CUT family (CUT, FastCUT, CPT, ASP) shares the same high-level
architecture: a ResNet generator ``netG``, a patch-sampling projection
network ``netF``, and a PatchGAN discriminator ``netD``. They differ only
in which auxiliary losses are applied during training.
"""
...
class BaseCUTModel(nn.Module, StainModel):
    """Common CUT-family model wrapper.

    Subclasses set hyperparameters (e.g. ``nce_idt``, ``lambda_NCE``,
    ``lambda_gp``) and are registered under distinct names in
    ...
    def _build_networks(self, no_d: bool) -> None:
        """Construct ``self.netG``, ``self.netF`` and optionally ``self.netD``."""
    ...
    def forward(
        ...
        Args:
            source_image: Input image tensor in ``[-1, 1]``.
            encode_only: If True, return intermediate encoder features instead
                of the generated image. Used by the NCE loss.
            layers: Layer indices to extract when ``encode_only=True``.

        Returns:
            :class:`StainOutput` with ``pred_image`` set to the generated image,
            or with auxiliary ``feats`` when ``encode_only=True``.
        """
        layers = layers if layers is not None else []
        if encode_only:
            feats = self.netG(source_image, layers=layers, encode_only=True)
            return StainOutput(pred_image=source_image, auxiliary={"feats": feats})
        pred = self.netG(source_image)
        return StainOutput(pred_image=pred)

    def get_trainable_params(self) -> list[nn.Parameter]:
        """Return all trainable parameters (G + F + D if present)."""
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "netF": self.netF.__class__.__name__,
            "netD": self.netD.__class__.__name__ if self.netD else None,
        }


class BaseCUTProcessor(StainProcessor):
    """Common processor for CUT-family image-to-image translation.
...
__all__ = ["BaseCUTModel", "BaseCUTProcessor"]
