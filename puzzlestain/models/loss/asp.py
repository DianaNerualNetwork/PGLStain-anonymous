"""Adaptive supervised PatchNCE loss from the ASP repository.

Original source: https://github.com/lifangda01/AdaptiveSupervisedPatchNCE
...
class AdaptiveSupervisedPatchNCELoss(nn.Module, StainLoss):
    """PatchNCE loss with an adaptive reweighting schedule.

    When ``asp_loss_mode`` is ``"none"`` the loss reduces to standard
    PatchNCELoss. Otherwise the mode string should be
    ``"<scheduler>_<lookup>"`` (e.g. ``"lambda_linear"``), which selects a
    scheduling curve and a lookup-based per-sample weight.

    Args:
        nce_T: Temperature for the InfoNCE softmax.
        batch_size: Minibatch size used to reshape negatives.
        nce_includes_all_negatives_from_minibatch: If True, treat every patch
            in the full minibatch as a negative for every query patch.
    ...
    def forward(
        ...
        """Compute the adaptive supervised PatchNCE loss.

        Returns:
            A tensor of shape ``(num_patches,)`` with the unreduced loss for
            each query patch.
        """
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        """Compute the mean adaptive supervised PatchNCE loss.

        ``current_epoch`` is extracted from ``kwargs`` when present so the loss
        can be used both directly and inside :class:`CompositeLoss`.
        """
        if feat_q is None or feat_k is None:
            raise ValueError(
                "AdaptiveSupervisedPatchNCELoss requires 'feat_q' and 'feat_k' kwargs"
            )
...
LossRegistry.register("asp", AdaptiveSupervisedPatchNCELoss)
